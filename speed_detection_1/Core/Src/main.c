...
// --- GPIOピン読み取り用マクロ ---
#define HALL_A_READ() HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_0)
#define HALL_B_READ() HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_1)
#define HALL_C_READ() HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_2)
...
uint16_t Get_Duty_From_ADC()
{
  ...
  // 12bit ADC (0〜4095) を PWM の Period 値にスケーリング
  return (uint16_t)((adc_val * htim1.Init.Period) / 4095);
}
